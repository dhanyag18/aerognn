...
class PhysicsLoss(torch.nn.Module):
    ...
    def momentum_residual(self, velocity, pressure, pos,
        ...
        R_x = conv_x + (1/self.rho) * grad_p[:, 0, 0:1] - self.nu * laplacian_u[:, 0:1]
        R_y = conv_y + (1/self.rho) * grad_p[:, 0, 1:2] - self.nu * laplacian_u[:, 1:2]
        R_z = conv_z + (1/self.rho) * grad_p[:, 0, 2:3] - self.nu * laplacian_u[:, 2:3]
