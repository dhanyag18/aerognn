...
def test_circle_when_n_equals_2():
    cs = generate_cross_section(n_1=2, n_2=2, n_3=2, m=4, num_points = 36)
    radii = np.sqrt(cs[:, 0]**2 + cs[:, 1]**2)
    assert np.allclose(radii, radii[0], atol=1e-6)


def test_square():
    cs = generate_cross_section(n_1=500, n_2=500, n_3=500, m=4, num_points = 360)
    radii = np.sqrt(cs[:, 0]**2 + cs[:, 1]**2)
    r_min = np.min(radii)
    r_max = np.max(radii)
...
def test_aspect_ratio():
    cs = generate_cross_section(n_1=1000, n_2=1000, n_3=1000, m=4, num_points = 36)
...
def test_area_normalization():
    cs = generate_cross_section(n_1=10, n_2=10, n_3=10, m=3, num_points = 36)
    cs_scaled = apply_aspect_ratio(cs, 2)
...
def test_height():
    cs = generate_cross_section(n_1=10, n_2=10, n_3=10, m=3, num_points = 36)
    vert, face = extrude_building(num_layers=20, bulge=1, taper=1, helical_twist= 0, num_setbacks=0, setback_reduction=0, chamfer_distance=0, cross_section=cs)
    max_z = np.max(vert[:, 2])
